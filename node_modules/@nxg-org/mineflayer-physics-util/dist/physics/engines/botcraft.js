...
class BotcraftPhysics {
    ...
    applyMovement(ctx, world) {
        ...
        if (Math.pow(player.stuckSpeedMultiplier.norm(), 2) > 1e-7) {
    ...
    // localPlayer.java
    isCollidedHorizontallyMinor(state, var1) {
        if (!(state instanceof states_1.PlayerState))
            return false;
        if (this.verGreaterThan("1.20.3")) { // apparently 1.20.4+ {
            ...
            const horizSpeed = Math.pow(xxaRot, 2) + Math.pow(zzaRot, 2);
    ...
    checkInsideBlocks(player, world) {
            ...
            for (blockPos.x = Math.floor(minAABB[0]); blockPos.x <= Math.floor(maxAABB[0]); ++blockPos.x) {
                for (blockPos.z = Math.floor(minAABB[2]); blockPos.z <= Math.floor(maxAABB[2]); ++blockPos.z) {
                    ...
                    if (this.webId === block.type) {
                        ...
                        player.stuckSpeedMultiplier = new vec3_1.Vec3(0.25, 0.05000000074505806, 0.25);
                    }
                        ...
                        player.stuckSpeedMultiplier = new vec3_1.Vec3(0.800000011920929, 0.75, 0.800000011920929); // magic number
                    }
                    else if (this.powderSnowId === block.type) {
                        // PowderSnowBlock::entityInside
                        const feetBlock = world.getBlock(new vec3_1.Vec3(player.pos.x, player.pos.y, player.pos.z));
                        if (feetBlock && this.powderSnowId === feetBlock.type) {
                            player.stuckSpeedMultiplier = new vec3_1.Vec3(0.8999999761581421, 1.5, 0.8999999761581421);
                        }
                    }
                }
            }
   ...
   */
    ...
    getSupportingBlockPos(world, feetSliceAABB) {
        ...
        for (blockPos.y = Math.floor(minAABB[1] - 1); blockPos.y <= Math.floor(maxAABB[1]); ++blockPos.y) {
            for (blockPos.x = Math.floor(minAABB[0]); blockPos.x <= Math.floor(maxAABB[0]); ++blockPos.x) {
                for (blockPos.z = Math.floor(minAABB[2]); blockPos.z <= Math.floor(maxAABB[2]); ++blockPos.z) {
                    ...
                    for (const shape of block.shapes) {
                        const bb1 = mineflayer_util_plugin_1.AABB.fromShape(shape).translateVec(blockPos);
    ...
    simulate(entity, world) {
        var _a;
        var _b;
        (_a = (_b = entity.state).attributes) !== null && _a !== void 0 ? _a : (_b.attributes = {});
        this.physicsTick(entity, world);
        entity.state.age++;
        return entity.state;
    }
}
exports.BotcraftPhysics = BotcraftPhysics;
